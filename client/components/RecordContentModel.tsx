...
interface RecordContentModalProps {
  isOpen: boolean;
  onClose: () => void;
}
...
export default function RecordContentModal({ isOpen, onClose }: RecordContentModalProps) {
  ...
  const checkPermissions = async (type: RecordingType) => {
    try {
      ...
      // For screen capture, we can't check permissions beforehand, so return true
      return true;
    } catch (error) {
      // If permissions API is not available, assume we can try
      return true;
    }
  };
  ...
  const startRecording = async () => {
    try {
      ...
      if (selectedType === 'camera') {
        ...
        // For screen capture, try with different options if the first fails
        try {
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden">
        <DialogHeader>
          <div className="flex items-center justify-between">
            <div>
              <DialogTitle className="text-xl font-semibold">Record Content</DialogTitle>
              <DialogDescription>
                Record video, audio, or screen content for your course
              </DialogDescription>
            </div>
            <Button
              variant="ghost"
