...
interface Institution {
  ...
  name: string;
  code: string;
  type: 'university' | 'college' | 'Government' | 'Aided' | 'Self-Finance' | 'Autonomous';
...
const ProgramAllotmentEdit: React.FC = () => {
  ...
  const [formData, setFormData] = useState<ProgramAllotment>({
    id: "",
    programId: "",
    institutionId: "",
    academicYear: "",
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        if (!programResponse.ok) throw new Error(`HTTP ${programResponse.status}: Failed to fetch programs`);
        const programData = await programResponse.json();
        if (Array.isArray(programData)) {
          const activePrograms = programData.filter((program: Program) => program.status === 'active');
      ...
      try {
        ...
        const institutionResponse = await fetch('/entity.json');
        if (!institutionResponse.ok) throw new Error(`HTTP ${institutionResponse.status}: Failed to fetch institutions`);
        const institutionData = await institutionResponse.json();
        if (Array.isArray(institutionData)) {
          const activeInstitutions = institutionData.filter((institution: Institution) => institution.status === 'active');
          setInstitutions(activeInstitutions);
        } else {
          throw new Error('Invalid data format: Expected array of institutions');
        }
      } catch (error) {
        console.error('Error loading institutions:', error);
        const errorMessage = error instanceof Error ? error.message : 'Failed to load institutions';
      ...
      // Load sample allotment data - updated to match polytechnic college data
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success("Program allotment updated successfully");
      navigate("/master/program-allotment");
    } catch (error) {
      toast.error("Failed to update program allotment");
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="container mx-auto p-6">
        <div className="text-center py-12">
          ...
          <p className="mt-1 text-sm text-gray-500">
            The program allotment you're trying to edit doesn't exist.
          </p>
          <div className="mt-6">
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Edit Program Allotment</h1>
            <p className="text-gray-600 mt-1">
              Update the program allotment details and settings
            </p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <Badge className={`${getStatusColor(formData.status)} flex items-center gap-1 px-3 py-1`}>
          ...
          >
            ...
            {isLoading ? "Updating..." : "Update Allotment"}
          </Button>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Update the program and institution for the allotment
              </CardDescription>
            </CardHeader>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <SearchableSelect
                  ...
                  emptyText={
                    ...
                    institutionsError ? "Error loading institutions" :
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Update the academic year and intake capacity
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => handleInputChange('intake', parseInt(e.target.value) || 0)}
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Update the validity period and approval status
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
            ...
            <CardContent className="space-y-4">
              {selectedProgram && (
                <div className="p-3 bg-blue-50 rounded-lg">
                  <h4 className="font-medium text-blue-900">Selected Program</h4>
                  <p className="text-sm text-blue-700">{selectedProgram.name}</p>
                  <p className="text-xs text-blue-600">
                    {selectedProgram.code} • {selectedProgram.level} • {selectedProgram.department}
                  </p>
                  <p className="text-xs text-blue-500 mt-1">
                    Duration: {selectedProgram.duration} years • Credits: {selectedProgram.totalCredits}
                  </p>
                </div>
              )}
              ...
              {selectedInstitution && (
                <div className="p-3 bg-green-50 rounded-lg">
                  <h4 className="font-medium text-green-900">Selected Institution</h4>
                  <p className="text-sm text-green-700">{selectedInstitution.name}</p>
                  <p className="text-xs text-green-600">
                    {selectedInstitution.code} • {selectedInstitution.type} • {selectedInstitution.district}
                  </p>
                  <p className="text-xs text-green-500 mt-1">
                    Address: {selectedInstitution.address_line1} • PIN: {selectedInstitution.pincode}
                  </p>
                </div>
              )}
              ...
              {formData.academicYear && (
                <div className="p-3 bg-purple-50 rounded-lg">
                  ...
                  <p className="text-sm text-purple-700">{formData.academicYear}</p>
                </div>
              )}

              {formData.intake && formData.intake > 0 && (
                <div className="p-3 bg-orange-50 rounded-lg">
                  <h4 className="font-medium text-orange-900">Intake Capacity</h4>
                  <p className="text-sm text-orange-700">{formData.intake} students</p>
                </div>
              )}
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              <Button
                ...
                onClick={() => navigate(`/master/program-allotment/view/${formData.id}`)}
              >
...
export default ProgramAllotmentEdit;
